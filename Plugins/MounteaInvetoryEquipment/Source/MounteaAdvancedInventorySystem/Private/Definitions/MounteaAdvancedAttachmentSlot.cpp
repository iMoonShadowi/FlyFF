...
{
    // Unwrap TObjectPtr to raw pointer to avoid ?: ambiguity
    USceneComponent* const Override = AttachmentTargetComponentOverride.Get();
    ...
    // Guard the interface pointer before calling Execute_*
    UObject* const ContainerObj = ParentContainer.GetObject();
    return ContainerObj
        ? ParentContainer->Execute_GetAttachmentTargetComponent(ContainerObj)
        : nullptr;
}
